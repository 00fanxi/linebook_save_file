...
var app = http.createServer(function(request,response){
					...
					fs.readFile(`./linebook.css`,'utf8',function(err,CSS){
						fs.readFile(`./CDF_E1.css`,'utf8',function(err,CDF_E1){
							fs.readFile(`./linebook.js`,'utf8',function(err,JS){
								...
								var HTML = template.HTML(title,CSS,CDF_E1,JS,list,data,newportform);
								response.writeHead(200);
								response.end(HTML);
							});
						});
	...
	//새로운 포트 만들기
		...
		request.on('end',function(){
			...
			<input type="text" class="ref r${i}" style="display:none;"></div>`
						...
						<input type="text" class="ref r${i}" style="display:none;"></div></div>`
								}else{
									plus_port += `
			<div class="CDF_E1_row"><span id="nUm" class="num n${i}">${i}</span><input type="text" class="usage u${i}" style="display:none;">
			<input type="text" class="ref r${i}" style="display:none;"></div>`
	...
	<input type="text" class="ref r${i}" style="display:none;"></div>`
					...
					var port_width = 1.71*Math.ceil(port_number/4);
					...
					fs.readFile(`./connection/connection.js`,'utf8',function(err,JS){
						fs.readdir('./data',function(error,filelist){
							...
							fs.readFile(`./connectdata/${center_name}`,'utf8',function(err,log_file){
								var HTML = template.connect_control(CSS,JS,filelist,west_opp,east_opp,center_name,west_id,east_id,west_port,east_port,log_file);
								response.writeHead(200);
								response.end(HTML);
							});
						});
