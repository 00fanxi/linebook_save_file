...
//메인화면 HTML
template.HTML = function(title,CSS,CDF_E1,JS,list,data,newportform){
	return `
	<!DOCTYPE html>
<html lang="en">
<head>
...
template.CDF_E1 = function(port_name,thisnode,oppnode,cable_type,numin,plus_ports,port_number,port_width){
	...
	function spread_p${numin}(self){
		if(self.value === '펼쳐보기'){
			if(${port_number}<33){
				document.querySelector("#p${numin}").style.width = 13*Math.ceil(${port_number}/4) + 'rem';
				document.querySelector("#p${numin} .portsinport").style.width = 13*Math.ceil(${port_number}/4) + 'rem';
			}else if(${port_number}<65){
				document.querySelector("#p${numin}").style.width = '104rem';
				document.querySelector("#p${numin} .portsinport").style.width = '104rem';
			}
...
//연결관리
template.connect_control = function(CSS,JS,filelist,west_opp,east_opp,center_name,west_specific,east_specific,west_port,east_port,log_file){
